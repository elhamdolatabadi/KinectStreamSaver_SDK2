...
void ExtractRotationInDegree(float buffer[4], double* pitch, double* yaw, double* roll)
{
...
int StreamSaver:: SaveSkeletonStream(bool EnStop_Skel)
{
	...
	JointOrientation_Data TempOreintationBuffer[BODY_COUNT];
...
//	{
		...
		if (!SkeletonBuffer.empty())
		{
			for (int i = 0; i < BODY_COUNT; i++)
			{
				...
				TempOreintationBuffer[i] = SkeletonBuffer.front().SkeletonOrientation[i];
		...
		{		
			...
			float qbuffer[JointType_Count*BODY_COUNT][4] = { 0, 0 };
			float qbuffer_Euler[JointType_Count*BODY_COUNT][3] = { 0, 0 };

			for (int i = 0; i < BODY_COUNT; i++)
			{	
				Joint pJoints[JointType_Count];
				JointOrientation qJoints[JointType_Count];
				...
				for (int j = 0; j < JointType_Count; j++)
				{
					...
					qJoints[j] = TempOreintationBuffer[i].JointOrientations[j];

					// jointPosition & jointOrientation
					if (pJoints[j].TrackingState != TrackingState_Inferred && pJoints[j].TrackingState != TrackingState_Tracked)
					{
						...
						qJoints[j].Orientation.x = 0;
						qJoints[j].Orientation.y = 0;
						qJoints[j].Orientation.z = 0;
						qJoints[j].Orientation.w = 0;
					}
					// jointPosition
					...
					// jointOrientation
					qbuffer[k][0] = qJoints[j].Orientation.x;
					qbuffer[k][1] = qJoints[j].Orientation.y;
					qbuffer[k][2] = qJoints[j].Orientation.z;
					qbuffer[k][3] = qJoints[j].Orientation.w;
					...
					qbuffer_Euler[k][0] = pitch;
					qbuffer_Euler[k][1] = yaw;
					qbuffer_Euler[k][2] = roll;

					/*ExtractRotationInDegree(qbuffer[k], pitch, yaw, roll);
					int qbuffer_Euler[JointType_Count*BODY_COUNT][3] = { 0, 0 };
					qbuffer_Euler[k][0] = *pitch;
					qbuffer_Euler[k][1] = *yaw;
					qbuffer_Euler[k][2] = *roll;*/

					/*SkeletonOrientEuler << pitch << "," << yaw << "," << roll << endl;
					SkeletonOrientEuler << x << "," << y << "," << z << "," << w << "," << endl;
					SkeletonOrientEuler << jbuffer[k][0] << "," << jbuffer[k][1] << "," << jbuffer[k][2] << "," << jbuffer[k][3] << "," << endl;
					SkeletonOrientEuler << endl;*/
				}
			}	
			...
			if (SkeletonJoint && SkeletonTime)
			{
				...
				SkeletonOrient.write((const char*)(qbuffer), JointType_Count * BODY_COUNT * 4 * sizeof(float));
				SkeletonOrientEuler.write((const char*)(qbuffer_Euler), JointType_Count * BODY_COUNT * 3 * sizeof(int));
...
void StreamSaver:: OpenSkeletonFile(WCHAR foldername[MAX_PATH])
{
	...
	StringCchPrintfW(SkeletonFileName, _countof(SkeletonFileName), L"%s\\Joint_Orientation_EulerAngles.binary", foldername);
	SkeletonOrientEuler.open(SkeletonFileName, ios::binary);
